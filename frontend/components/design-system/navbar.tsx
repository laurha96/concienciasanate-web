...
export function Navbar({
  ...
  return (
    <header
      className={cn(
        "sticky top-0 z-50 w-full border-b border-white/15 bg-white/[0.06] shadow-none backdrop-blur-none supports-[backdrop-filter]:bg-white/[0.04]",
        className
      )}
  >
      ...
      <nav className="hidden items-center gap-6 md:flex">
        {items.map((item) => (
          <Link
            key={item.href}
            href={item.href}
            className="-mx-3 -my-2 rounded-full px-3 py-2 text-sm font-medium tracking-tight text-text-secondary transition-colors hover:bg-white/30 hover:text-text-primary"
          >
            {item.label}
          </Link>
        ))}
